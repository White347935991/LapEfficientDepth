...
class Res_Efficient(nn.Module):
    def __init__(self,
        ...
        super(Res_Efficient, self).__init__()
        ...
        self.resnet = ResNet(BasicBlock, [3, 3])

        model_cnf2 = [[2, 3, 1, 6, 128, 160, 1, 0.25],
                      [2, 3, 2, 6, 160, 256, 1, 0.25]]
...
def myModel(num_classes: int = 1000):
    model = Res_Efficient(num_classes=num_classes, dropout_rate=0.2)
    return model
...
if __name__=='__main__':
    ...
    map = torch.rand(1, 3, 300, 300)

    # 可视化内容
    print('Start Tensorboard with "tensorboard --logdir=runs", view at http://localhost:6008/')
    tb_writer = SummaryWriter('./logs')

    model = myModel()
    # tb_writer.add_graph(model, input_to_model=map)
    # tb_writer.close()
    print(model)