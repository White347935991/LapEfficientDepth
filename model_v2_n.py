...
from EfficientV2_B0 import Res_Efficient
...
class deepFeatureExtractor_EfficientNetV2(nn.Module):
    def __init__(self, args, lv6=False):
        super(deepFeatureExtractor_EfficientNetV2, self).__init__()
        self.args = args
        self.lv6 = lv6
        self.encoder = Res_Efficient()
        self.dimList = [64, 128, 256, 512]

    def forward(self, x):
        out_featList = []
        # # 计算需要在高度（第三个维度）上增加的总单位数
        # total_padding = 448 - x.size(2)
        # # 计算每侧需要增加的单位数
        # padding_each_side = total_padding // 2
        # # 使用pad函数进行填充，格式为 (左, 右, 上, 下)
        # x = F.pad(x, (0, 0, padding_each_side, padding_each_side), "constant", 0)

        # x1 = self.encoder.blocks1(self.encoder.stem(x))
        print(f'x.shape: {x.shape}')
        x1 = self.encoder.blocks1(x)
        x2 = self.encoder.resnet.layer1(x1)
        x3 = self.encoder.resnet.layer2(x2)
        x4 = self.encoder.blocks2(x3)

        out_featList.append(x1)
        out_featList.append(x2)
        out_featList.append(x3)
        out_featList.append(x4)
...
class Lap_decoder_lv5(nn.Module):
    ...
    def forward(self, x, rgb):
        ...
        if len(x) >= 4:
            ...
            # 处理列表长度不足的情况
            print(f'len(x): {len(x)}')
            raise ValueError("List 'x' does not have enough elements.")
        ...
        # lap_lv4 = torch.tanh(self.decoder2_4(dec2) + (0.1 * rgb_lv5.mean(dim=1, keepdim=True)))
        # tmp = selorch.tanh(tmp_up + (0.1 * rgb_lv4.mean(dim=1, keepdim=True)))
        ...
        # dec5 = self.decoder2_1_1_1_1(torch.cat([dec5, lap_lv2_up, rgb_lv2], dim=1))
        dec5 = self.decoder2_1_1_1_2(dec5)
        lap_lv1 = torch.tanh(self.decoder2_1_1_1_3(dec5) + (0.1 * rgb_lv1.mean(dim=1, keepdim=True)))
        # lap_lv1 = torch.tanh(self.decoder2_1_1_1_3(dec5) + (0.1 * rgb_lv2.mean(dim=1, keepdim=True)))
        # if depth range is (0,1), laplacian of image range is (-1,1)
        ...
        # 特征图扩大2倍
        final_depth = self.upscale(final_depth, scale_factor=2, mode='nearest-exact')
...
class LDRN(nn.Module):
    def __init__(self, args):
        ...
        #     self.encoder = deepFeatureExtractor_EfficientNet(args, encoder, lv6)
            ...
            self.encoder = deepFeatureExtractor_EfficientNetV2(args, lv6)
    ...
    def forward(self, x):
        ...
        # rgb_down2 = F.interpolate(x, scale_factor=0.25, mode='bilinear')
...
if __name__=='__main__':
    from option import args
    model = LDRN(args)
    # print(model.encoder.encoder.features)

    # 创建一个随机输入张量
    test_input = torch.randn(1, 3, 224, 224)  # 假设输入尺寸为 224x224
    print(model)

    # 前向传播
    with torch.no_grad():  # 确保不计算梯度
        output = model(test_input)
        print("前向传播成功！输出尺寸:", output)
