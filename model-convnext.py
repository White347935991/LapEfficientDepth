...
# Deep Feature Fxtractor
class DeepFeatureExtractor_ConvNeXt_tiny(nn.Module):
    def __init__(self, args, lv6=False):
        ...
        # 根据 lv6 参数选择是否包含所有层
        if lv6:
            self.layerList = ['0', '2', '4', '6']
            self.dimList = [96, 192, 384, 768, 1536]  # 确保列表长度足够
        else:
            del self.encoder.features[3]  # 删除最后一层
            self.layerList = ['0', '2', '4', '6']
            self.dimList = [96, 192, 384, 768]  # 或者根据实际情况调整
        ...
        for name, module in self.encoder.named_children():
            if name == 'features':
                for sub_name, sub_module in module.named_children():
                    ...
                    if any(x in sub_name for x in self.layerList):
                        out_featList.append(feature)
            else:
                break  # 如果不是features层，结束循环
        return out_featList
...
class LDRN(nn.Module):
    def __init__(self, args):
        ...
        if encoder == 'ResNext101':
            ...
            self.encoder = DeepFeatureExtractor_ConvNeXt_tiny(args, lv6)
