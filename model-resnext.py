...
class DeepFeatureExtractor_ConvNeXt_tiny(nn.Module):
    def __init__(self, lv6=False):
        ...
        # 根据 lv6 参数选择是否包含所有层
        if lv6:
            self.layerList = ['features.0', 'features.1', 'features.2', 'features.3']
            self.dimList = [96, 192, 384, 768]  # ConvNeXt-tiny的维度列表
        else:
            del self.encoder.features[3]  # 删除最后一层
            self.layerList = ['features.0', 'features.1', 'features.2']
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        # 将ConvNeXt-tiny分解为多个层次
        self.layer1 = nn.Sequential(*list(self.convnext.children())[0][0:3])
        self.layer2 = nn.Sequential(*list(self.convnext.children())[0][3:6])
        self.layer3 = nn.Sequential(*list(self.convnext.children())[0][6:9])
        self.layer4 = nn.Sequential(*list(self.convnext.children())[0][9:12])

    def forward(self, x):
        # 获取每个层次的输出，这些输出将用于解码器的shortcut连接
        x1 = self.layer1(x)
        x2 = self.layer2(x1)
        x3 = self.layer3(x2)
        x4 = self.layer4(x3)
...
class deepFeatureExtractor_EfficientNet(nn.Module):
    def __init__(self, args, architecture="EfficientNet-B5", lv6=False):
        ...
        if architecture == "EfficientNet-B0":
            ...
            self.encoder = geffnet.tf_efficientnet_b0_ns(pretrained=False)
            ...
            if k == 'blocks':
                for m, n in v._modules.items():
                    ...
                    if self.block_idx[block_cnt] == cnt:
                        ...
                        block_cnt += 1
                    cnt += 1
                    if block_cnt >= len(self.block_idx):
                        break
